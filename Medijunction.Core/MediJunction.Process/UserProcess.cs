    ...
    public class UserProcess : IUserProcess
    {
        ...
        public ReconsultationResponse Reconsultation(ReConsultationRequest reconsultationRequest)
        {
            if (reconsultationRequest != null)
            {
                ...
                {
                    return new ReconsultationResponse { IsSuccessStatusCode = false, Message = "Error : " + ex.Message, StatusCode = System.Net.HttpStatusCode.InternalServerError };
                    //response = Request.CreateResponse(HttpStatusCode.OK, new HttpResult { Message = "Error : " + ex.Message, Response = false, Result = ex.Message });
                }
            }
            else
            {
